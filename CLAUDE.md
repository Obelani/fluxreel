...
- `api/_lib/imagePrompt.js` — monta o prompt final de cada cena (`buildImagePrompt`): bloco de formato/cena/personagens/universo + regras universais de consistência + prompt completo do estilo (`visualStyles.js`) + prompt negativo universal + regras negativas do estilo. `validateImagePromptInput` roda antes de cada chamada à API de imagem. A fal.ai/z-image/turbo (modelo usado aqui) **não tem campo de `negative_prompt` nem aceita imagem de referência** — só `seed` é suportado de verdade (confirmado na doc da API), por isso o "negativo" é só texto dentro do mesmo prompt.
